...
object VisitAggregation extends App {
  ...
  val outputWithStartVisitNumberGuid = outputWithStartVisit.withColumn("visit_number_guid", expr("last_value(start_visit, true)").over(Window.partitionBy("guid").orderBy("timestamp")))

  //outputWithStartVisitNumberGuid.select($"guid", $"timestamp", $"gmt", $"previous_gmt", $"previous_lag", $"start_visit", $"visit_number_guid").show(false )

  val visitDF = outputWithStartVisitNumberGuid.withColumn("visit_start_timestamp",expr("min(from_unixtime(gmt, 'yyyy-MM-dd HH:mm:ss'))").over(Window.partitionBy("guid","visit_number_guid"))).drop("gmt","previous_gmt","previous_lag","start_visit", "visit_number_guid")

  visitDF.select($"guid", $"timestamp",$"visit_start_timestamp").show()
